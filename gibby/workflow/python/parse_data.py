...
from gibby.utils.process_data import parse_finite_difference_files, is_edge_list_respected, is_adsorbate_adsorbed_and_unreacted
import pandas as pd
import pickle
...
if __name__ == "__main__":
    ...
    parser.add_argument("--traj_path", type=str, help="Path to the oc20 trajectory files")
    ...
    parser.add_argument("--tag_mapping_path", type=str, help="Path to the tag mapping file")
    ...
    for adsorbate in adsorbates:
        smile_lookup[adsorbates[adsorbate][1]] = (len(adsorbates[adsorbate][0]), adsorbates[adsorbate][4])    
    ...
    for folder in tqdm(glob(f"{args.data_path}/**")):
        ...
        entry = {"mapping_idx": int(idx), "completed": completed, "hessian": hessian, "atoms": atoms, "atoms_list": atoms_list}
    ...
    df["adsorbate_len"], df["adsorbate_edges"] = zip(*df.adsorbate.apply(get_adsorbate_info))

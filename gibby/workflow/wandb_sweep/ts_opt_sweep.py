...
def get_parser():
    ...
    parser.add_argument("--sweep_mode", type=str, default="init", help="init or agent")
...
def main(config):
    ...
    if config["sweep_mode"] == "init":
        sweep_config = {
            ...
            "method": "random",
            "metric": {"goal": "minimize", "name": "fmax"},
            "parameters": {
                "neb_path": {
                    "values": [
        ...
        wandb.agent(
            config["sweep_id"], function=run_sweep, project=config["sweep_name"]
        )
